...
export default function AdminPanelScreen({ navigation }) {
  ...
  const { t } = useI18n();
  const { adminPlace, adminQueue, adminNext, confirmResetQueue, adminLogout } = useApp();
  ...
  return (
    <View style={[styles.fill, { backgroundColor: colors.bgGradient[0] }]}>
      <LinearGradient colors={colors.bgGradient} style={styles.fill}>
        <HeaderBar
          title={`${adminPlace.name.split(" ")[0]} · Admin`}
          onBack={onBack}
          showThemeToggle={false}
          right={
            <TouchableOpacity
              onPress={() => navigation.navigate("Display")}
              style={[styles.screenBtn, { backgroundColor: colors.inputBg, borderColor: colors.inputBorder }]}
            >
              <Ionicons name="tv" size={14} color={colors.accent} />
              <Text style={[styles.screenBtnText, { color: colors.accent, fontFamily: fonts.bold }]}>{t("adminDisplayBtn")}</Text>
            </TouchableOpacity>
          }
        />
        ...
        <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 40 }}>
          ...
          <View style={styles.statsRow}>
            <GlassCard style={styles.statBox}>
              <TouchableOpacity onPress={() => setServedOpen(true)} style={styles.statTouchable}>
...
const styles = StyleSheet.create({
  fill: { flex: 1 },
  screenBtn: { flexDirection: "row", alignItems: "center", gap: 5, paddingVertical: 8, paddingHorizontal: 12, borderRadius: 12, borderWidth: 1 },
  screenBtnText: { fontSize: 12.5 },
  ...
  statTouchable: { padding: 15, alignItems: "center" },
  statVal: { fontSize: 24 },
  statLbl: { fontSize: 11.5, marginTop: 2 },
